...
- TLD（Top-Level Domain，顶层域名）
- SLD（Second-Level Domain，次级域名）
...
### ✔ 完全数据驱动，所有推断规则来自外部数据文件：

- RootZoneDatabase.csv   ← 根域数据库分类，基于[IANA官方](https://www.iana.org/domains/root/db),并结合[TLD WIKI](https://en.wikipedia.org/wiki/List_of_Internet_top-level_domains)和[工信部中国互联网域名体系](https://domain.miit.gov.cn/)修改
- RootZoneDatabase.txt   ← 根域数据库文本，基于[IANA官方](https://data.iana.org/TLD/tlds-alpha-by-domain.txt)转成小写。
...
- **pseudo-sld** — 伪二级域名构造器（全球通用，如 `com.cn`、`net.jp`）
...
- [工信部中国互联网域名体系](https://domain.miit.gov.cn/) ，删除与 pseudo-sld 相同部分，实现减少 I/O：
    - ac
    - com
    - edu
    - gov
...
- [工信部中国互联网域名体系](https://domain.miit.gov.cn/) ，保留了省级 sld:
    |sld:cn|省级名称|
    |---|---|
    |bj|北京市|
    |tj|天津市|
...
📊 性能表现 (Benchmark)
- 测试环境：普通 NVMe SSD / 标准多核 CPU
- 数据集： [Domcop Top 10 Million Domains](https://www.domcop.com/files/top/top10milliondomains.csv.zip)。
...
# IANA 官方数据 https://data.iana.org/TLD/tlds-alpha-by-domain.txt
# 使用之前，把内容转为小写
...
TLD 校验（对照 RootZoneDatabase.txt）
    ...
    └─ ──────── 已知 TLD
     ...
     命中                    未命中
       │                      │
       ▼                      ▼
 提取domain.3         保留在 domain.2
  （3 标签）             （2 标签）
```

| 输入 | 输出 | 路径 |
|---|---|---|
| `www.example.com` | `example.com` | 命中 original → domain.2 |
...
对所有匹配 `\.cn$` 的域名，**由 pseudo-sld 处理了部分，省级域名由 pseudo-sld:cn 从 `domain.3` 提取策略。**
